...
class FastVARSelfAttention(nn.Module):
    ...
    def kv_caching(self, enable: bool, history_cache_max_len: int = -1): # kv caching: only used during inference
        self.caching = enable
        self.cached_k = None
        self.cached_v = None
        self.history_cache_max_len = int(history_cache_max_len)
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0,rope_idx=None,ori_len=0):
        ...
        if self.caching:    # kv caching: only used during inference
            if self.cached_k is None:
                ...
                if self.history_cache_max_len is not None and self.history_cache_max_len >= 0:
                    prev_k = self.cached_k
                    prev_v = self.cached_v
                    keep_len = min(self.history_cache_max_len, prev_k.shape[L_dim])
                    if keep_len > 0:
                        prev_k = prev_k.narrow(L_dim, prev_k.shape[L_dim] - keep_len, keep_len)
                        prev_v = prev_v.narrow(L_dim, prev_v.shape[L_dim] - keep_len, keep_len)
                    else:
                        prev_k = prev_k.narrow(L_dim, 0, 0)
                        prev_v = prev_v.narrow(L_dim, 0, 0)
                    k = self.cached_k = torch.cat((prev_k, k), dim=L_dim)
                    v = self.cached_v = torch.cat((prev_v, v), dim=L_dim)
                else:
                    k = self.cached_k = torch.cat((self.cached_k, k), dim=L_dim)
                    v = self.cached_v = torch.cat((self.cached_v, v), dim=L_dim) # 10,521
...
class FastVARCrossAttnBlock(nn.Module):
    ...
    def forward(
        ...
        is_later_layer = True if layer_idx in list(range(1,32)) else False
        can_update_previous_scale_cache = (
            x.shape[1] == (self.cached_size[0] * self.cached_size[1])
            and (not disable_cache_update_for_reuse or x.shape[1] == (self.cached_size[0] * self.cached_size[1]))
        )
        merge_fn, unmerge_fn, idx_fn = compute_merge(
            x,
        ...
        if can_update_previous_scale_cache:
