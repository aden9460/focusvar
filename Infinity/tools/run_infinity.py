...
CURRENT_ROOT = osp.dirname(osp.dirname(osp.abspath(__file__)))
...
def gen_one_img(
    ...
    enable_fastvar_compute_merge=0,
    warmup_runs=0,
    print_timing=1,
    fixed_history_cache_len=-1,
    force_cond_only_branch=0,
    skip_final_two_scales=0,
    cond_only_start_scale=-1,
    # EXPERIMENT FLAG: enable layerwise boundary collapse to cond-only path.
    enable_layerwise_cond_only_collapse=0,
    cond_only_start_layer=-1,
    enable_adaptive_cond_only_collapse=0,
    adaptive_stats_only=0,
    ...
    if fixed_history_cache_len >= 0:
        print(f'fixed_history_cache_len={fixed_history_cache_len}')
    if force_cond_only_branch:
        print('force_cond_only_branch=1')
    if cond_only_start_scale >= 0:
        print(f'cond_only_start_scale={cond_only_start_scale}')
    if enable_layerwise_cond_only_collapse:
        print(f'enable_layerwise_cond_only_collapse=1, cond_only_start_layer={cond_only_start_layer}')
    if enable_adaptive_cond_only_collapse or adaptive_stats_only:
        print(
            f'enable_adaptive_cond_only_collapse={int(bool(enable_adaptive_cond_only_collapse))}, '
            f'adaptive_stats_only={int(bool(adaptive_stats_only))}, probe_scale={adaptive_probe_scale}, '
            f'probe_layer={adaptive_probe_layer}, late_layer={adaptive_late_collapse_layer}, '
            f'top_p={adaptive_top_p_percent}, tau_A={adaptive_tau_A}, tau_g={adaptive_tau_g}, tau_d={adaptive_tau_d}'
        )
    if not skip_final_two_scales:
        print('skip_final_two_scales=0')
    infinity_test.enable_fastvar_compute_merge = bool(enable_fastvar_compute_merge)
    if use_shared_update and shared_update_policy is not None:
        print(f'shared_update_policy={shared_update_policy}')
    print(
        f'pruning_mode fastvar={int(bool(getattr(infinity_test, "enable_fastvar_compute_merge", False)))} '
        f'spacevar={int(bool(getattr(infinity_test, "enable_spacevar_compute_merge", False)))} '
        f'keep_ratio_by_scale={getattr(infinity_test, "fastvar_ratio_by_scale", {})}'
    )
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        _, intermidiate_list, img_list = infinity_test.autoregressive_infer_cfg(
            ...
            print_timing=bool(print_timing),
            fixed_history_cache_len=fixed_history_cache_len,
            ...
            # EXPERIMENT FLAG: layerwise collapse after one CFG fusion at boundary.
            ...
            enable_cond_only_token_reuse=bool(enable_cond_only_token_reuse),
            cond_only_token_reuse_source=str(cond_only_token_reuse_source),
            cond_only_token_reuse_policy=str(cond_only_token_reuse_policy),
            ...
            scale_decode_output_dir=scale_decode_output_dir,
            scale_decode_max_scales=int(scale_decode_max_scales),
            branch_decode_output_dir=branch_decode_output_dir,
            branch_decode_scale_index=int(branch_decode_scale_index),
        )
...
def build_shared_update_policy(scale_start: int, scale_end: int, layer_start: int, layer_end: int):
    if scale_start < 0 or scale_end < scale_start:
        raise ValueError(f'invalid shared update scale range: {scale_start}-{scale_end}')
    if layer_start < 0 or layer_end < layer_start:
        raise ValueError(f'invalid shared update layer range: {layer_start}-{layer_end}')
    return {si: [(layer_start, layer_end)] for si in range(scale_start, scale_end + 1)}
...
def load_infinity(
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True), torch.no_grad():
        ...
        print(f'[you selected Infinity with model_kwargs={model_kwargs}] model size: {sum(p.numel() for p in infinity_test.parameters())/1e9:.2f}B, bf16={bf16}')
...
def add_common_arguments(parser):
    ...
    parser.add_argument('--use_shared_update', type=int, default=0, choices=[0,1])
    parser.add_argument('--shared_update_scale_start', type=int, default=5)
    parser.add_argument('--shared_update_scale_end', type=int, default=10)
    parser.add_argument('--shared_update_layer_start', type=int, default=16)
    parser.add_argument('--shared_update_layer_end', type=int, default=27)
    parser.add_argument('--enable_fastvar_compute_merge', type=int, default=1, choices=[0,1])
    parser.add_argument('--enable_spacevar_compute_merge', type=int, default=0, choices=[0,1])
    parser.add_argument('--fastvar_ratio_by_scale', type=str, default='32:0.4,40:0.5')
    parser.add_argument('--warmup_runs', type=int, default=1)
    parser.add_argument('--print_timing', type=int, default=1, choices=[0,1])
    parser.add_argument('--fixed_history_cache_len', type=int, default=-1)
    parser.add_argument('--force_cond_only_branch', type=int, default=0, choices=[0,1])
    parser.add_argument('--skip_final_two_scales', type=int, default=1, choices=[0,1])
    parser.add_argument('--cond_only_start_scale', type=int, default=-1)
    # EXPERIMENT FLAG: collapse from dual-branch to cond-only inside a scale.
    ...
    parser.add_argument('--enable_adaptive_cond_only_collapse', type=int, default=0, choices=[0,1])
    parser.add_argument('--adaptive_stats_only', type=int, default=0, choices=[0,1])
    parser.add_argument('--adaptive_probe_scale', type=int, default=8)
    parser.add_argument('--adaptive_probe_layer', type=int, default=5)
    parser.add_argument('--adaptive_late_collapse_layer', type=int, default=20)
    parser.add_argument('--adaptive_top_p_percent', type=float, default=10.0)
    parser.add_argument('--adaptive_tau_A', type=float, default=1.0)
    parser.add_argument('--adaptive_tau_g', type=float, default=0.0)
    parser.add_argument('--adaptive_tau_d', type=float, default=0.0)
    parser.add_argument('--enable_cond_only_token_reuse', type=int, default=0, choices=[0,1])
    parser.add_argument('--cond_only_token_reuse_source', type=str, default='cfg_diff', choices=['cfg_diff'])
    parser.add_argument('--cond_only_token_reuse_policy', type=str, default='freeze_once', choices=['freeze_once'])
    parser.add_argument('--visualize_token_entropy', type=int, default=0, choices=[0,1])
    parser.add_argument('--token_entropy_output_dir', type=str, default='')
    parser.add_argument('--token_entropy_overlay_alpha', type=float, default=0.45)
    parser.add_argument('--token_entropy_save_raw', type=int, default=0, choices=[0,1])
    parser.add_argument('--token_entropy_save_mode', type=str, default='all', choices=['all','entropy_gap_partial'])
...
if __name__ == '__main__':
    ...
    infinity.enable_fastvar_compute_merge = bool(args.enable_fastvar_compute_merge)
    infinity.enable_spacevar_compute_merge = bool(args.enable_spacevar_compute_merge)
    infinity.fastvar_ratio_by_scale = {
        int(item.split(':', 1)[0]): float(item.split(':', 1)[1])
        for item in args.fastvar_ratio_by_scale.split(',') if item
    }
    
    scale_schedule = dynamic_resolution_h_w[args.h_div_w_template][args.pn]['scales']
    scale_schedule = [ (1, h, w) for (_, h, w) in scale_schedule]
    shared_update_policy = None
    if args.use_shared_update:
        shared_update_policy = build_shared_update_policy(
            scale_start=args.shared_update_scale_start,
            scale_end=args.shared_update_scale_end,
            layer_start=args.shared_update_layer_start,
            layer_end=args.shared_update_layer_end,
        )
    ...
    with autocast(dtype=torch.bfloat16):
        with torch.no_grad():
            generated_image = gen_one_img(
                ...
                use_shared_update=args.use_shared_update,
                shared_update_policy=shared_update_policy,
                enable_fastvar_compute_merge=args.enable_fastvar_compute_merge,
                warmup_runs=args.warmup_runs,
                print_timing=args.print_timing,
                fixed_history_cache_len=args.fixed_history_cache_len,
                force_cond_only_branch=args.force_cond_only_branch,
                skip_final_two_scales=args.skip_final_two_scales,
                cond_only_start_scale=args.cond_only_start_scale,
                enable_layerwise_cond_only_collapse=args.enable_layerwise_cond_only_collapse,
                cond_only_start_layer=args.cond_only_start_layer,
                enable_adaptive_cond_only_collapse=args.enable_adaptive_cond_only_collapse,
                adaptive_stats_only=args.adaptive_stats_only,
                adaptive_probe_scale=args.adaptive_probe_scale,
                adaptive_probe_layer=args.adaptive_probe_layer,
                adaptive_late_collapse_layer=args.adaptive_late_collapse_layer,
                adaptive_top_p_percent=args.adaptive_top_p_percent,
                adaptive_tau_A=args.adaptive_tau_A,
                adaptive_tau_g=args.adaptive_tau_g,
                adaptive_tau_d=args.adaptive_tau_d,
                enable_cond_only_token_reuse=args.enable_cond_only_token_reuse,
                cond_only_token_reuse_source=args.cond_only_token_reuse_source,
                cond_only_token_reuse_policy=args.cond_only_token_reuse_policy,
                visualize_token_entropy=args.visualize_token_entropy,
                token_entropy_output_dir=args.token_entropy_output_dir,
                token_entropy_overlay_alpha=args.token_entropy_overlay_alpha,
                token_entropy_save_raw=args.token_entropy_save_raw,
                token_entropy_save_mode=args.token_entropy_save_mode,
            )
