...
from hart.modules.networks.fastvar_utils import (
    set_fastvar_compute_merge_enabled,
    set_fastvar_prune_scales,
    set_fastvar_start_layer,
    set_fastvar_verbose,
    set_spacevar_config,
)
...
class HARTForT2I(PreTrainedModel):
    ...
    def __init__(self, config: HARTForT2IConfig):
        ...
        self.hart_enable_fastvar_compute_merge = False
        self.hart_enable_spacevar_compute_merge = False
        self.hart_fastvar_ratio_by_scale = {48: 0.4, 64: 0.5}
        self.hart_fastvar_prune_scales = {48, 64}
        self.hart_fastvar_start_layer = 0
        self.hart_enable_layerwise_cond_only_collapse = False
        self.hart_cond_only_start_scale = -1
        self.hart_cond_only_start_layer = -1
        self.hart_fastvar_verbose = False

    def configure_inference_acceleration(
        self,
        enable_fastvar_compute_merge=False,
        enable_spacevar_compute_merge=False,
        fastvar_ratio_by_scale=None,
        fastvar_prune_scales=None,
        fastvar_start_layer=0,
        enable_layerwise_cond_only_collapse=False,
        cond_only_start_scale=-1,
        cond_only_start_layer=-1,
        fastvar_verbose=False,
    ):
        self.hart_enable_fastvar_compute_merge = bool(enable_fastvar_compute_merge)
        self.hart_enable_spacevar_compute_merge = bool(enable_spacevar_compute_merge)
        if fastvar_ratio_by_scale is not None:
            self.hart_fastvar_ratio_by_scale = {int(k): float(v) for k, v in fastvar_ratio_by_scale.items()}
        if fastvar_prune_scales is not None:
            self.hart_fastvar_prune_scales = {int(s) for s in fastvar_prune_scales}
        self.hart_fastvar_start_layer = int(fastvar_start_layer)
        self.hart_enable_layerwise_cond_only_collapse = bool(enable_layerwise_cond_only_collapse)
        self.hart_cond_only_start_scale = int(cond_only_start_scale)
        self.hart_cond_only_start_layer = int(cond_only_start_layer)
        self.hart_fastvar_verbose = bool(fastvar_verbose)
    ...
    def autoregressive_infer_cfg(
        ...
        for b in self.blocks:
            ...
            b.previous_scale_cache_attn = None
            b.previous_scale_cache_ffn = None

        set_fastvar_verbose(self.hart_fastvar_verbose)
        set_fastvar_compute_merge_enabled(self.hart_enable_fastvar_compute_merge)
        set_spacevar_config(self.hart_enable_spacevar_compute_merge, self.hart_fastvar_ratio_by_scale)
        set_fastvar_prune_scales(self.hart_fastvar_prune_scales)
        set_fastvar_start_layer(self.hart_fastvar_start_layer)
        ...
        for si, pn in enumerate(self.patch_nums[:-1]):  # [1, 2, 3, 4, 5, 7, 9, 12, 16, 21, 27, 36, 48, 64]
            ...
            if x.shape[0] == B:
                ...
                context_position_ids = context_position_ids[:B]
                context_mask = context_mask[:B]
            FastVARAdaLNSelfAttn.forward
            current_scale = pn
            collapse_active_for_scale = (
                self.hart_enable_layerwise_cond_only_collapse
                and self.hart_cond_only_start_scale >= 0
                and current_scale >= self.hart_cond_only_start_scale
            )

            collapsed_to_cond_only_this_scale = x.shape[0] == B
            for layer_idx, b in enumerate(self.blocks): # [24 x blocks]
                should_layerwise_cond_only_collapse = (
                    ...
                    and self.hart_cond_only_start_layer >= 0
                    and layer_idx >= self.hart_cond_only_start_layer
                    and x.shape[0] == 2 * B
                )
                if should_layerwise_cond_only_collapse:
                    t = cfg * ratio
                    print(f"[hart-layerwise-cond-only] collapse at scale={si} ({pn}x{pn}) layer={layer_idx} cfg_t={t}")
                    x = ((1 + t) * x[:B] - t * x[B:]).contiguous()
                    cond_BD_or_gss = cond_BD_or_gss[:B]
                    context_position_ids = context_position_ids[:B]
                    context_mask = context_mask[:B]
                    for cache_idx, cache_block in enumerate(self.blocks):
                        if cache_idx < layer_idx:
                            continue
                        if cache_block.attn.cached_k is not None:
                            cache_block.attn.cached_k = cache_block.attn.cached_k[:B].contiguous()
                        if cache_block.attn.cached_v is not None:
                            cache_block.attn.cached_v = cache_block.attn.cached_v[:B].contiguous()
            ...
            if x.shape[0] == B:
                logits_BlV = self.get_logits(x, cond_BD[:B])
                if si == 0:
        ...
        current_bs = next_token_map.shape[0]
        cond_only_final = current_bs == B
        if cond_only_final:
            cond_BD_or_gss = cond_BD_or_gss[:B]
            context_position_ids = context_position_ids[:B]
            context_mask = context_mask[:B]
        ...
        for step in indices:
            ...
            mask = mask_next
            cur_mask = mask_to_pred if cond_only_final else torch.cat([mask_to_pred, mask_to_pred], dim=0)
            cur_mask = cur_mask.nonzero(as_tuple=True)
            x = next_token_map[cur_mask].reshape(current_bs, -1, self.C)
            ...
            if final_stage == 0:
                ...
                if cond_only_final:
                    ...
                    h_BChw_diff = self.diffloss.sample(
                        z=last_stage_cond, temperature=1.0, cfg=1.0
                    )
